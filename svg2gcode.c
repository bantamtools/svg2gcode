...
typedef struct {
  float points[8];
  int shape; //corresponds to a shape id, as defined in the Shape datastructure. Want to refactor this to use shape ID's from NSVGShape.
  char closed;
} ToolPath;
  ...
  for (shape = g_image->shapes; shape != NULL; shape = shape->next) { //ID of our shape type = order encountered.
    for (path = shape->paths; path != NULL; path = path->next) { //For each path in shape.
      ...
      if(hasFill(shape) && (fillShapeInsert < fillShapeCount)){
        fprintf(debug, "    Fill Shape %i, Shape ID: %s, Bounds %f %f %f %f. Fill: %i\n", fillShapeInsert, shape->id, shape->bounds[0], shape->bounds[1], shape->bounds[2], shape->bounds[3], shape->fill.type);
        fflush(debug);
...
void writePoint(FILE * gcode, FILE* color_gcode, GCodeState * gcodeState, TransformSettings * settings, BVHNode * bvh, int * ptIndex, char * isClosed, int * machineType, int * sp, int * pathPointIndex, int* pointsWritten, int* i, int* k) {
    float feedRate;
    float dist = 0.0;
    ...
    if(canWritePoint(gcodeState, settings, sp, ptIndex, pathPointIndex, &x, &y, gcode, &writeReason)){ //Can write, if first or last in shape, or if dist is large enough.
      gcodeState->xold = gcodeState->x; //Update state tracking if we are writing. Stored x and y become old x and y.
      gcodeState->yold = gcodeState->y;
      gcodeState->x = x; //New points into gcodeState
      gcodeState->y = y;
      dist = distanceBetweenPoints(gcodeState->xold, gcodeState->yold, x, y);

      if(!firstPoint(sp, ptIndex, pathPointIndex)){ //Intermediary Pointlogic.
...
void writeShape(FILE * gcode, FILE* color_gcode, GCodeState * gcodeState, TransformSettings * settings, BVHNode * bvh, Shape * shapes, ToolPath * toolPaths, int * machineTypePtr, int * k, int * i) { //k is index in toolPaths. i is index i shapes.
    ...
    if(sp){ //If endpoint is closer.
      for(int z = pathPointsIndex-2; z >= 0; z-=2){ //write backwards if sp, forwards if else.
        ...
        writePoint(gcode, color_gcode, gcodeState, settings, bvh, &z, &isClosed, machineTypePtr, &sp, &pathPointsIndex, &pointsWritten, i, k);
      }
...
int generateGcode(int argc, char* argv[], int** penColors, int penColorCount[6], float paperDimensions[9], int generationConfig[14], char* fileName) {
  ...
  //Bank of pens, their slot and their color(s). Pens also track count of shapes to be drawn with their color (for debug purposes)
  ...
  TransformSettings settings = calcTransform(g_image, paperDimensions, generationConfig, bounds);
 ...
 if(writeBuffer == NULL){
  ...
  //Need to check for mem leaks eventually.
  fflush(stdout);
  fclose(gcode);
  fclose(debug);
  if(fillShapes->size > 0){
